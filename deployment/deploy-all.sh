...
# ============================================================================ #
# Project Pulse / مشاريعنا Unified Deployment Script
# ============================================================================ #
# This script combines functionality from all three deployment scripts:
# 1. deploy.sh - Full remote deployment to Ubuntu 24.04 servers
# 2. deploy-new.sh - Local deployment for development/testing
# 3. render-deploy.sh - Deployment specific to Render platform
#
# Usage: ./deploy-all.sh [local|remote|render]
#
...
setup_local_postgres() {
  ...
  if [ -f "$PROJECT_ROOT/check-database.js" ]; then
    ...
    if [ -f "$PROJECT_ROOT/server/init-database.sh" ]; then
      bash "$PROJECT_ROOT/server/init-database.sh"
    else
      log_error "Could not find any database initialization scripts."
      return 1
    fi
  fi
...
deploy_render() {
  ...
  log_info "Installing dependencies including Vite..."
  cd "$PROJECT_ROOT"
...
if [ "$#" -eq 1 ]; then
  case "$1" in
    ...
    *) echo "Invalid argument. Usage: ./deploy-all.sh [local|remote|render]" ;;
  esac
else
  echo "Invalid usage. Usage: ./deploy-all.sh [local|remote|render]"
fi 